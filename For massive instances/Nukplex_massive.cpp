...
void fast_init_solution(){
	...
	while( cur_splex->vnum < param_s){
		...
		{
			best_time = clock();
			cout << param_s << param_graph_file_name << " " << param_seed << " " << red_vnum << " " << (float)(best_time - start_time) / CLOCKS_PER_SEC << endl;
			exit(0);
		}
...
void tabu_based_search(){
	...
	while (1){
		...
		while (!end){
			...
			if (non_improve_iter > param_s * best_size2){
				non_improve_iter = 0;
				core_perturb_flag = 1;
			}
			cur_iter++;
		...
		int v_remove = cur_splex->vlist[rand() % cur_splex->vnum];
		remove_cur_vertex(v_remove);
...
void report_result(){
	cout << param_s << param_graph_file_name << " " << param_seed << " " << best_size << " " << (float)(best_time - start_time) / CLOCKS_PER_SEC << endl;
}
